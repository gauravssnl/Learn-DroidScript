...
function MakeDrawer()
{
  ...
  var fVal=Math.round(2*Math.pow(window.localStorage['lower-cutoff'],0.66667));
...
//  freqThres = app.CreateText( "Exclude 1024 most common words" );
  ...
  if(markThreshVal==undefined){markThreshVal=10}
...
//  app.ShowPopup(layouthash[window.localStorage['layout']]); 
    ...
    if(side=="Left" && state=="Closed" && basicSetChange==1){
       web.Execute("location.reload();");
       window.localStorage['markThresh']=markThreshVal;
      //     app.ShowPopup('drawer just shut? ' + window.localStorage['markThresh'])
       setMarks();
    }
}
